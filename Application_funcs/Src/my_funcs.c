...
void init_buff(void){
  ...
  rand_time = conv256to50(*((uint8_t*)&temp_rnd));  // время
  rand_size = conv256to50(*((uint8_t*)&temp_rnd + 1));  // размер
  rand_buff[0] = *((uint8_t*)&temp_rnd + 2);  // 0-й элемент
  rand_buff[1] = *((uint8_t*)&temp_rnd + 3);  // 1-й элемент
}
